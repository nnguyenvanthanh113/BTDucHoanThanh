        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item has-treeview menu-open">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="create_lct.php" class="nav-link active">
...
<?php 
  ...
  if(isset($_GET['id']))
  {
    $id = $_GET['id'];
    //var_dump($id);
    $sqli = "SELECT * FROM lichcongtac WHERE Id='".$id."'";
        $result = mysqli_query($conn,$sqli) or die('could not seclect');
         if(mysqli_num_rows($result)==0)
            echo "không tồn tại lịch công tác của nhân viên  !";
          else
          {
            while ($row = mysqli_fetch_array($result))
            {
  ...
  if(isset($_POST['ngaydi'])) $ngaydi=$_POST['ngaydi'];
  if(isset($_POST['ngayve'])) $ngayve=$_POST['ngayve'];
  if(isset($_POST['noicongtac'])) $noicongtac=$_POST['noicongtac'];
  if(isset($_POST['phuongtien'])) $phuongtien=$_POST['phuongtien'];
  if(isset($_POST['noidung'])) $noidung=$_POST['noidung'];
  if(isset($_POST['xoa']))
  {
      $sql = "DELETE FROM lichcongtac
            
            WHERE Id='$id'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_query($conn, $sql))
        {
          echo "<script>alert('Xóa lịch công tác thành công')</script>";            
        }
        else
          echo "<script>alert('Xóa lịch công tác thất bại')</script>";
        echo "<script>location.replace('lichcongtacnhanvien.php');</script>";
  }
                        ...
                        <div class="col-lg-12 col-md-12  col-sx-12">
                          <form action="#" method="post">
                              <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                  <input type="date" class="form-control" name="ngaydi" value="<?php echo $ngaydi;?>" required="required" readonly>
                                </div>
                              </div>
                              <div class="form-group row">
                                <label for="inputEmail3" class="col-sm-2 col-form-label">Ngày về</label>
                                <div class="col-sm-4">
                                  <input type="date" class="form-control" name="ngayve" value="<?php echo $ngayve; ?>" required="required" readonly>
                                </div>
                              </div>
                              <div class="form-group row">
                                <label for="inputEmail3" class="col-sm-2 col-form-label">Nơi công tác</label>
                                <div class="col-sm-4">
                                  <input type="text" class="form-control" name="noicongtac" value="<?php echo $noicongtac; ?>" placeholder="nơi công tác" required="required" readonly>
                                </div>
                              </div>
                              <div class="form-group row">
                                <label for="inputEmail3" class="col-sm-2 col-form-label">Phương tiện di chuyển</label>
                                <div class="col-sm-4">
                                  <input type="text" class="form-control" name="phuongtien" value="<?php echo $phuongtien; ?>" placeholder="phương tiện di chuyển" required="required" readonly>
                                </div>
                              </div>
                              <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                   <div class="form-group">
                                        <textarea class="form-control rounded-0" name="noidung" rows="3" required="required" readonly><?php echo $noidung;?></textarea>
                                   </div>
                                </div>
                              </div>
                              <div class="form-group row">
                                  <div class="col-2">
                                    <button type="submit" name="xoa" class="btn btn-primary btn-block">Xóa</button>
                                  </div>
                              </div>
                          </form>
                        </div>
